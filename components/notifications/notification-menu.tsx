    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button 
          variant="ghost" 
          size="icon" 
          className="relative h-9 w-9 text-foreground/70 hover:text-foreground hover:bg-accent/50 transition-colors"
        >
          <Bell className="h-5 w-5" />
          {unreadCount > 0 && (
            <Badge
...
function NotificationItem({ notification }: { notification: NotificationData }) {
  const typeIcon = typeIcons[notification.type as NotificationType] || "🔔"
  const priorityColor = priorityColors[notification.priority as NotificationPriority] || "text-muted-foreground"
